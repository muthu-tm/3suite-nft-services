...
export const UploadFileToIPFS = async function (file) {
    try {
...
export const UploadMetaToIPFS = async function (content) {
    try {
        const formData = new FormData();
        formData.append('file', content);

        const auth = 'Basic ' + Buffer.from(infuraConfig.ipfs.api_key + ':' + infuraConfig.ipfs.api_secret).toString('base64');
        const resp = await axios.post(infuraConfig.ipfs.api_url + "/api/v0/add", content, {
            headers: {
                'Authorization': auth,
            },
        });
