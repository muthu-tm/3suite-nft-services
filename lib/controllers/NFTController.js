...
export default class NFTController extends Controller {
    ...
    async getUserCreated(request) {
        try {
            if (!request.user) {
                return this.handleException(new UnauthorizedException("Please login to access this resource!"))
            }
            let data = await db.nft_assets.fetch_nft_by_creator_address(request.user.address)
            return this.sendResponse({
                status: true,
                error: {},
                data: data,
    ...
    async getUserOwned(request) {
        try {
            ...
            let data = await db.nft_assets.fetch_nft_by_owner_address(request.user.address)
            return this.sendResponse({
                status: true,
                error: {},
                data: data,
