...
import AccountsQuery from "../../db/query/user_accounts.query.js";
...
export default class UserController extends Controller {
  ...
  async verify(request) {
    try {
      ...
      let account = await this.user_accounts.fetch_user_by_account(address, chain_id)
      if (account === null) {
        ...
        return this.handleException(
          new Exceptions.ClientException("Address already exists")
        );
      }
    } catch (error) {
  ...
  async register(request, response, next) {
    try {
      ...
      let address = request.body.address;
      let chain_id = request.body.chain_id;
      let chain_name = request.body.chain_name || "Polygon";
      let type = request.body.type || "ERC20";
      let profile_img = "";
      ...
      if (user === null) {
        ...
        let wallet = await db.user_accounts.create({
          address: address,
          chain_id: chain_id,
          name: chain_name,
          type: type
