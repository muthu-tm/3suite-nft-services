  ...
  async verifyUserID(request) {
    try {
      ...
      if (user.length == 0) {
        ...
        return this.handleException(
          new Exceptions.ClientException("User ID already exists")
        );
      }
    } catch (error) {
      return this.handleException(error);
      ...
      request.logIn(user, { session: false }, async function (err) {
        ...
        try {
          if (request.files.image) {
            const fileContent = Buffer.from(request.files.image.data, "binary");
            let resp_data = await aws_utils.upload_file(
              fileContent,
            ...
            const result = await db.user.update(
              user.id,
              { "profile.img": resp_data.Location },
              true
            );

            user = await db.user.fetch_user_by_id(user.id);
          }
        } catch (err) {
